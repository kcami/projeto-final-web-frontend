...
export default function Legend(props: {pageType: number, componentText: string}){
    const legenda = [
        {
          color: styles.green,
    ...
    return(
        ...
        <Container className={`${styleLegenda.legenda} w-100 bg-blueSecondary`} >
                {props.pageType == 1 && <ListGroup className={`${styleLegenda.listGroup} w-100`} variant='blueSecondary'>
                    {legenda.map((element, i) => {
                        return (

                            <ListGroup.Item key={i} className={`${styleLegenda.itemLegenda}`}>
